...
def project(request, project_id):
	if project_id.isdigit():
		...
		project_obj = get_object_or_404(Project, idname=project_id)

	context = {'project': project_obj, 'isauthor': False}
	...
	if request.user.is_authenticated():
		if request.method == 'POST':
			...
			if context['form'].is_valid():
				...
				context['form'].save(user=request.user, project=project_obj, replyto=replyto)
				messages.success(request, 'Your comment has been submitted')
		context['form'] = CommentForm()

		if request.user.project_set.filter(pk=project_obj.pk).exists():
			context['isauthor'] = True

	context['comments'] = Comment.objects.filter(project=project_obj, replyto=None)
...
def project_edit(request, project_id):
	if project_id.isdigit():
		...
		project_obj = get_object_or_404(Project, idname=project_id)
	if not request.user.is_authenticated or not request.user.project_set.filter(pk=project_obj.pk).exists():
	...
	if request.method == 'POST':
		form = ProjectForm(instance=project_obj, data=request.POST, files=request.FILES)
		if form.is_valid():
			form.clean()
			form.save()
			return HttpResponseRedirect(reverse('project', args=[project_obj.idname]))

	return render(request, "mainpage/project_edit.html", {'project': project_obj, 'form': form})


def project_download(request, project_id):
	project_obj = get_object_or_404(Project, Q(pk=project_id) | Q(idname=project_id))

	project_obj.downloads += 1
	project_obj.save()

	wrapper = FileWrapper(open(project_obj.file.path, "rb"))
	response = HttpResponse(wrapper, content_type='application/zip')
	response['Content-Disposition'] = 'attachment; filename=%s' % smart_str(os.path.basename(project_obj.file.name))
	response['Content-Length'] = os.path.getsize(project_obj.file.path)
	return response
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			messages.success(request,
			                 'You have successfully registered. Please check your mail and click the activation link')
			return HttpResponseRedirect(reverse('index'))
